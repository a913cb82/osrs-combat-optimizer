...
MOCK_WEAPON_DB = {
    "super weapon": {"atk_req": 1, "str": 100, "speed": 2.4, "cost": 10.0, "bonus": {"stab": 100, "slash": 100, "crush": 100}},
    "okay weapon":  {"atk_req": 1, "str": 50,  "speed": 2.4, "cost": 10.0, "bonus": {"stab": 50,  "slash": 50,  "crush": 50}},
}

class TestGraphPruningBug(unittest.TestCase):
    def test_pruning_ignores_graph_costs(self):
        """
        ...
        with patch('optimize_melee.AMULETS_DB', MOCK_AMMY_DB), \
            ...
            result = optimize_melee.solve(
                req_graph=graph,
                goal_atk=10, goal_str=10,
                start_atk=1, start_str=1,
                timeout=5, lookahead=100
            )
            ...
            self.assertLess(total_time, 50000, "Optimizer picked the expensive path! Pruning bug confirmed.")

...
def generate_xp_table(max_level: int = 126) -> dict[int, int]:
    ...
    return {l: get_xp_for_level(l) for l in range(1, max_level + 1)}
...
def calculate_max_hit(str_lvl: int, equip_str_bonus: int, style_bonus: int) -> int:
    ...
    Args:
        str_lvl: Visible Strength level (including potions/prayers, though we assume base).
    ...
    base_damage = 0.5 + (effective_str * (equip_str_bonus + EQUIPMENT_BONUS_OFFSET)) / MAX_HIT_DIVISOR
    return math.floor(base_damage)

def calculate_hit_chance(atk_lvl: int, equip_atk_bonus: int, style_bonus: int, 
                         monster_def_lvl: int = 1, monster_def_bonus: int = 0) -> float:
    """
